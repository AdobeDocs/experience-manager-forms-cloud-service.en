...
* **post.POST.jsp**: The Submit servlet calls this script with the data that you submit and the additional data in the previous sections. Any mention of running an action in this page implies running the post.POST.jsp script. To register the Submit action with the adaptive forms to display in the Adaptive Form Edit dialog, add these properties to the sling:Folder:
    ...
    * **submitService** of type String and value. For more information, see [Schedule adaptive form submission for custom actions](#schedule-adaptive-form-submission).
...
Add the `submitService` property of type string to the `sling:Folder` of your custom submit action to enable Adobe Sign for the adaptive form. You can select the **[!UICONTROL Enable Adobe Sign]** option in the **[!UICONTROL Electronic Signature]** section of the adaptive form container properties only after setting the value for the `submitService` property of your custom submit action.

<!--As a result of setting an appropriate value for the `submitService` property and enabling Adobe Sign, you can schedule the submission of an adaptive form to ensure that all configured signers have taken an action on the form. Adobe Sign Configuration Service keeps polling Adobe Sign server at regular intervals to verify the status of signatures. If all the signers complete signing the form, the submit action service is started and the form is submitted.-->
